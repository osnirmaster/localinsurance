package br.com.spring.insurance;
...
public class GenerateComponents {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			ClassLoader.getSystemClassLoader().loadClass("org.springframework.samples.petclinic.model.Vet");
		} catch (ClassNotFoundException e) {
			System.err.println(
					"Please check that the compiled version of the Spring PetClinic application is on the classpath");
			System.exit(-1);
		}
		...
		ComponentFinder componentFinder = new ComponentFinder(webApplication, "org.springframework.samples.petclinic",
				new SpringComponentFinderStrategy(new ReferencedTypesSupportingTypesStrategy(false)),
		...
		for (Component component : webApplication.getComponents()) {
			for (CodeElement codeElement : component.getCode()) {
				...
				if (sourcePath != null) {
					codeElement.setUrl(sourcePath.replace(sourceRoot.toURI().toString(),
		...
		StructurizrClient structurizrClient = new StructurizrClient("d7e35495-5aa6-4a38-8eeb-520462ba47df",
				"b025d52a-a4ee-4ead-a8ea-563cfea8c947");
		structurizrClient.putWorkspace(56158, workspace);
